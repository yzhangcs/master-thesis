...
\begin{equation} \label{eq:biaffine-loss}
  \mathit{L}(i\rightarrow j) = -\log{\frac{\exp(\mathrm{s}(i\rightarrow j))}{\sum_{0 \le k \le n} \exp(\mathrm{s}(k,j))}}
\end{equation}
...
\begin{equation}
  ...
  \end{split}
\end{equation}
其中$Z(\boldsymbol{x}, {\boldsymbol{y}^p})$ 只包含所有与给定局部标注树兼容的句法树，并且可以以与$Z(\boldsymbol{x})$类似的方式进行高效的计算.
