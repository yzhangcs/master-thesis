...
\begin{tikzpicture}[
	...
	\node[anchor=base] at ($(mlp-1-2-f.north) + (0cm,0.5cm) $) (label-biaffine) {$\mathrm{s}(i\rightarrow j)$};
	\node[anchor=base] at ($(triaffine.north -| mlp-2-1-b) + (0,0.75cm) $) (label-triaffine) {$\mathrm{s}(i\rightarrow {k,j})$};
