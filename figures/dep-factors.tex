...
\begin{tikzpicture}[
    ...
    \node [circle,draw=gray, densely dashed, semithick,inner sep=0pt,minimum size=0.4cm]  (c0) at ($(w0.south) + (0,1.5cm)$) {};
    \foreach \x in {1,...,5} {
            \node [circle,draw=black,semithick,inner sep=0pt,minimum size=0.4cm]  (c\x) at ($(w0.south) + (1.5cm*\x,1.5cm)$) {};
    ...
    \draw[green connect] (c1) to [out=60, in=120] node[midway,green filled,inner sep=0pt,minimum size=0.2cm,rounded corners=0.3mm] {} (c4) ;
    \draw[green connect] (c2) to [out=60, in=120] node[midway,green filled,inner sep=0pt,minimum size=0.2cm,rounded corners=0.3mm] {} (c4) ;
    \draw[green connect] (c4) to [out=60, in=120] node[midway,green filled,inner sep=0pt,minimum size=0.2cm,rounded corners=0.3mm] {} (c5) ;

    % \draw [dep arrow] [out=-120,in=-60] ($(w2.south) + (-0.15, 0)$) to (w1.south);
    % \draw [dep arrow] [out=-60,in=-120] ($(w0.south) + (0.1, 0)$) to (w2.south);
    % \draw [dep arrow] [out=-60,in=-120] ($(w2.south) + (0.1, 0)$) to (w3.south);
    % \draw [dep arrow] [out=-60,in=-120] ($(w3.south) + (0.1, 0)$) to (w4.south);
    % \draw [dep arrow] [out=-60,in=-120] ($(w2.south) + (0.1, 0)$) to (w5.south);
    \node [red filled,inner sep=0pt,minimum size=0.2cm,rounded corners=0.3mm] (unary) at ($(w1.north)-(0,1.2cm)$) {};
    \node [anchor=west] (unary-text) at ($(unary.east)$) {Arc factor};
    \node [green filled,inner sep=0pt,minimum size=0.2cm,rounded corners=0.3mm] (binary) at ($(w3.north)-(0cm,1.2cm)$) {};
